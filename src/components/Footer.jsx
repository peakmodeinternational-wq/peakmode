...
export default function Footer() {
  return (
    <footer className="relative border-t border-white/10 bg-gradient-to-b from-navy to-[#101A2E]">
      <StripDivider />
          ...
          <div className="mt-6 flex items-center gap-3">
            {[
              ...
              <a
                ...
                href="#"
                aria-label={label}
                className="grid h-10 w-10 place-items-center border border-white/15 text-white/70 transition-all duration-300 hover:-translate-y-0.5 hover:border-gold/70 hover:text-gold-light hover:shadow-[0_10px_30px_-12px_rgba(217,180,91,0.55)]"
              >
        ...
        <div>
          ...
          <ul className="space-y-3">
            {NAV_GROUPS.products.map((item) => (
              <li key={item.name}>
                <Link to={item.href} className="text-sm text-white/70 transition-colors hover:text-gold">
                  {item.name}
                </Link>
              </li>
            ))}
